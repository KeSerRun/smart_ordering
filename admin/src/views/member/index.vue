<template>
  <n-tabs type="line" v-model:value="tab">
    <n-tab-pane name="members" tab="会员列表">
      <n-data-table :columns="mColumns" :data="members" :loading="loading" :scroll-x="1050" :pagination="mPagination" />
    </n-tab-pane>
  ...
  <n-modal v-model:show="showAssign" preset="card" :title="`设置会员等级 - ${assignTarget?.nickname || assignTarget?.username || ''}`" style="width:420px">
    <n-form label-placement="top">
      <n-form-item label="会员等级">
        <n-select v-model:value="assignForm.levelId" :options="levelOptions" placeholder="请选择等级" filterable />
      </n-form-item>
      <n-form-item label="当前等级">
        <n-text depth="3">{{ assignTarget?.levelName || '-' }}</n-text>
      </n-form-item>
    </n-form>
    <template #footer>
      <n-space justify="end">
        <n-button @click="showAssign=false">取消</n-button>
        <n-button type="primary" :loading="assigning" @click="saveAssign">确定</n-button>
      </n-space>
    </template>
  </n-modal>
</template>
...
const assignForm = ref({ levelId: null })
...
const levelOptions = computed(() => levels.value.map((l) => ({ label: l.levelName, value: l.id })))
...
// ===== 设置会员等级 =====
const openAssignLevel = (r) => {
  assignTarget.value = r
  assignForm.value = { levelId: r.levelId ?? null }
  showAssign.value = true
}
const saveAssign = async () => {
  if (!assignForm.value.levelId) {
    message.warning('请选择会员等级')
    return
  }
  assigning.value = true
  try {
    await assignMemberLevel(assignTarget.value.userId, { levelId: assignForm.value.levelId })
    message.success('等级设置成功')
    showAssign.value = false
    loadMembers()
  } catch (e) {
    message.error(e?.msg || '设置失败')
