...
import { getMemberGrowthRecords } from '@/api/member';
...
async function loadData(reset = false, withRefresh = false) {
  ...
  try {
    const result = await getMemberGrowthRecords({
      pageNum: nextPage,
    ...
    uni.showToast({ title: err.message || '加载成长明细失败', icon: 'none' });
  } finally {
